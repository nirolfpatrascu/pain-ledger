UNKNOWN_EVENTS={SPELLS_CHANGED=true, CHAT_MSG_ADDON=true}
...
function CreateFrame(kind,name)
  ...
  if not frameScript then
    ...
    rawset(f,"RegisterEvent",function(self,e) if UNKNOWN_EVENTS[e] then error('Attempt to register unknown event "'..e..'"') end events[e]=true end)
  end
...
-- test_events.lua: this mock client rejects some events, the way Classic Era
-- rejects events from newer clients. The addon must still set itself up.
money=0
fire("ADDON_LOADED","PainLedger"); fire("PLAYER_LOGIN"); flushAll()
eq(PainLedgerDB~=nil,true,"saved data set up despite rejected events")
eq(PainLedger.frame~=nil,true,"window built")
eq(#PainLedger.skippedEvents,2,"the two unknown events were skipped, nothing else")
eq(PainLedgerDB.chain~=nil and PainLedgerDB.chain.seq>0,true,"chain started")
